...
carte_plot <- function(crit,label,point,data){
    ...
    if(crit!=0){
        map <- map+geom_sf(data=bdd_zese,aes(fill=bdd_zese[,crit],geometry=geometry),size=.2,color=NA)+
            labs(fill = label)
    }else{
        map <-map+geom_sf(data=bdd_zese,aes(geometry=geometry),size=.2,color="blue",fill="black")
    }
    map <- map+
        scale_fill_viridis_c(option = 'E')+
        theme_minimal()+
        theme(panel.background = element_rect(fill = "light blue"))+
        geom_sf_text(data=bdd_zese,aes(label=`Zone d'emploi 2020`,geometry=geometry,color="white"),size=2)
    #choix de la coloration des banques
    if(point==0){
        map <- map+
        ...
        tabItems(
            ...
            tabItem("Map",
                    fluidRow(column(width=4,selectInput("choice",label="Choix de la representation : ",choices=c("Type","Banque"))),
                             column(width=4,selectInput("criteria",label="Choix du critere :", choices = c("0_Pas de critère",paste0(3:34,"_",colnames(bdd_zese[-c(1,2,35)])))))),
            ...
            tabItem("Comparaison-Type",
                    fluidRow(column(width=12,selectInput("Critere", label = "Choix du critere",choices =c("0_Pas de critère",paste0(3:34,"_",colnames(bdd_zese[-c(1,2,35)])))))),
            ...
            tabItem("Comparaison-Critere-Type",
                    ...
                    fluidRow(column(width=12,selectInput("Critere_1", label = "Choix du premier critere : ",choices = c("0_Pas de critère",paste0(3:34,"_",colnames(bdd_zese[-c(1,2,35)]))))),
                             column(width=12,selectInput("Critere_2", label = "Choix du deuxieme critere : ",choices = c("0_Pas de critère",paste0(3:34,"_",colnames(bdd_zese[-c(1,2,35)])))))),
...
server <- function(input,output){
    ...
    output$map_generale <- renderPlotly({
        ...
        carte_plot(crit,temp[2],point,data)
    })
